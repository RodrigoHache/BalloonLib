...
def create_multihead_model(trial: optuna.Trial) -> nn.Module:
    ...
    nv_fn_name = trial.suggest_categorical(
        "nv_fn", ["ELU", "LeakyReLU", "Softplus", "GELU", "SiLU"]
    )
    ...
    if use_scheduler:
        ...
        if sched_name == "StepLR":
            ...
            scheduler = optim.lr_scheduler.ExponentialLR(
                optimizer,
                gamma=trial.suggest_float("exp_gamma", 0.99, 0.9999, log=True),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--storage", type=str, default=None, help="SQLite URL, e.g. sqlite:///study.db"
    )
